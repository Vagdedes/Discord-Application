...
class DiscordUserQuestionnaire
{
                    ...
                    if ($query->create_channel_category_id !== null) {
                                ...
                                if ($question === true) {
                                    ...
                                    if (is_string($question)) {
                                        ...
                                        $message = $question->message_name !== null
                                            ? $this->bot->persistentMessages->get($object, $question->message_name)
                        ...
                        if ($channel !== null
                            ...
                            if ($question !== true) {
                                ...
                                $channel->startThread($message, $questionnaireID)->done($this->bot->utilities->oneArgumentFunction(
        ...
        if (strlen($message->content) > 0) {
            ...
            if (!empty($query)) {
                ...
                if (array_key_exists($query->questionnaire_id, $this->questionnaires)) {
                    ...
                    } else if ($message->member->id != $this->bot->botID) {
                            ...
                            if ($question === true) {
                                ...
                                if (is_string($question)) {
                                    $messageBuilder = MessageBuilder::new()->setContent(
                                        $this->bot->instructions->replace(array($question), $object)[0]
                                    );
                                } else {
    ...
    public function loadSingleQuestionnaireMessage(object $questionnaire): MessageBuilder
    {
        ...
        $embed = new Embed($this->bot->discord);
        $user = $this->bot->utilities->getUser($questionnaire->user_id);
    ...
    public function loadQuestionnaireMessage(int|string $userID, array $questionnaires): MessageBuilder
    {
        ...
        $messageBuilder = MessageBuilder::new();
        $messageBuilder->setContent("Showing last **" . sizeof($questionnaires) . " questionnaires** of user **" . $this->bot->utilities->getUsername($userID) . "**");
